...
z = z2(:,1);
  ...
  modetol = 1e-8; %changed to 1 from 1e-8 for testing TW

%Chris -- new Arguments that make changes simplier
jenny = 200;
amy = 180; %changed from 160 to 180 TW 9/27/16

  % solve for modes in parallel commenting this out to load in modes from previous runs - TW 2/27/20  % uncomment the block below to recalculate modes

  %tic()
  spmd(2,2)
    if (labindex == 1)
      [x,d,vr]=gmresdrEIG(M,z,jenny,amy,1e-160,750);% Chris -- jenny/amy, changed parameters here as below, TW 9/28/17
    elseif (labindex == 2)
      [xh,gd,gvr,ghk,gvk]=gmresdrEIG5(M,z,jenny,amy,1e-160,750); % Chris,jenny/amy, changed rtol to 1e-150 from 1e-50 and cyclim 
    end
   % toc()
%%%%%% end comment out TW 2/27/20

  %adding the below to get vecmodes!!! - TW 2/27/20
  %load(['vecmodes',ext,'-',num2str(latsize),'.mat']);
  end

  % store mode information from various nodes
  clear x xh %commented out since these arent stored in vecmodes
  d   = d{1};   vr  = vr{1};
  gd  = gd{2};  gvr = gvr{2};
  ghk = ghk{2}; gvk = gvk{2}; %these not saved in vecmodes, so commenting them out. they aren't used anyways, uncomment these if you need to regenerate the vecmodes - TW 2/27/20
