  ...
  !BS open(unit=32,file=trim(rwdir(myid+1))//"NSPSHFES.dat",status="old",     &
   ...
   do sty=1,10
     do iop=1,nop
       do isub=1,nsub
         do eig=1,numEV_MAX
            !if (sty==3 .and. iop==2 .and. isub==1 .and. eig==17) then
             ! print *,'HERE IS THE WEIRD DATA',sty,iop,isub,eig,it,Jtotal(1,it,isub,eig,sty,iop), &
             ! '  ',Jtotal(2,it,isub,eig,sty,iop)
            !endif
           do it=1,nt
             ! sty = 1  NS
             if (sty==1 .and. eig==1 .and. isub==1) then
...
if (.false.) then
 ...
 subroutine eigmodesofM(gxe,gxo,be,bo,evectorRevn,evectorRodd,evectorLevn,evectorLodd,eigValExpand,numEV, & !added g5xe/xo,be/bo TW 11/15/17
 ...
 real(kind=KR2),  dimension(6,ntotal,4,2,8)   :: xe,xo,x,wxo,wxe,xre,xro,wbe,b,g5xe,g5xo !be/bo were here TW                           
 real(kind=KR2),  intent(out),  dimension(6,ntotal,4,2,8) :: gxe,gxo,be,bo !TW added be/bo to intent out for gmresdr5EIG                                              
 integer(kind=KI)                         ::idag,ikappa,ntrue,ieo,ibleo,gblclr          
 real(kind=KR2),  dimension(2) :: tmp1,tmp2,tmp3,tmp4,xtmp1,xtmp2,xrn
 integer(kind=KI)  :: eig,eigpos,eigneg,eigrel,eigpair,eigprime
 integer(kind=KI)  :: iaa,j,i,icri
 ...
 if (myid == 0) then
   print *, 'entering eigmodesofM'
 endif

if (myid==0) then
 print *, 'The value of kappa in eigmodesofM is ',kappa(1)
endif
 ! GMRES parameters
 resmax = 1e-155_KR2

 !!!! -- Generate Eig-info from Mreduced -- !!!
! be(:6,:ntotal,:4,:2,:8) = 0.0_KR2   ! Arbitrary (non-zero) rhs, commented out TW 12/8/17           
! be(1,1,1,1,1) = 1.0_KR2             ! Arbitrary (non-zero) rhs, commented out TW 12/8/17             
 xe(:6,:ntotal,:4,:2,:8) = 0.0_KR2  ! Initial Guess

!**********added by TW 12/8/17***********!

 call z4source(be,bo,nvhalf,myid)

 gblclr = 1

 call Hsingle(wbe,u,bo,idag,coact,bc,gblclr,vecbl,vecblinv,myid,nn,ldiv,&
              nms,lvbc,ib,lbd,iblv,MRT)
  do j=1,nvhalf
   b(:,j,:,:,:) = (1.0_KR2/(kappa(1)**2))*be(:,j,:,:,:) &
                           + (1.0_KR2/kappa(1))*wbe(:,j,:,:,:) 
  enddo 
...
!print *,'evaleig',evalue
 ...
 !bo(:6,:ntotal,:4,:2,:8) = 0.0_KR2 !added TW 11/15/17
 !bo(1,1,1,1,1) = 1.0_KR2 

  gblclr = 2
  call Hsingle(tmpo,u,xe,idag,coact,bc, &
               gblclr,vecbl,vecblinv,myid,nn,ldiv,nms,lvbc,ib,lbd,iblv,MRT) !changed xe to x in input TW 12/10/17                            

   do j=1,nvhalf
     xo(:,j,:,:,:) = bo(:,j,:,:,:) + kappa(1)*tmpo(:,j,:,:,:) 
   enddo

 do ieo=1,2 !added TW 11/11/17, *******xo needs to be computed first!!!********
   do ibleo=1,8
      call gammamult(xe, xo ,g5xe, g5xo, 5,ieo,ibleo) !changed xe to x TW 12/10/17                 
   enddo
 enddo  !end add TW 11/15/17

 do ieo=1,2 !added TW 11/11/17, *******xo needs to be computed first!!!********
   do ibleo=1,8
      call gammamult(g5xe, g5xo ,gxe, gxo, 5,ieo,ibleo) !changed xe to x TW12/10/17
   enddo
 enddo  !end add TW 11/15/17
      ...
      call Hsingle(wxo,u,gxe,idag,coact,bc,gblclr,vecbl,vecblinv,myid,nn,ldiv,&
                      nms,lvbc,ib,lbd,iblv,MRT)
      ! Next build H_eo * v_o.
      gblclr = 1
      call Hsingle(wxe,u,gxo,idag,coact,bc,gblclr,vecbl,vecblinv,myid,nn,ldiv,&
                     nms,lvbc,ib,lbd,iblv,MRT)
      ...
      if (myid == 0) then
         print *, 'Residual for gxe/gxo is ', xtmp1,' ', xtmp2,' ',  xrn(1)
      endif
...
!BS *******printing of tmp2.dat ends here
 ...
 subroutine testFUNC(u,kappa,nrhs,coact,bc,vecbl,vecblinv,myid,ntmqcd,nn,ldiv, &
                     nms,lvbc,lvcl,ib,lbd,iblv,rwdir,MRT,MRT2,numprocs) !added inverter TW 11/17/15                                    
 real(kind=KR),    intent(inout), dimension(:,:,:,:,:) :: u
 real(kind=KR),    intent(in),    dimension(:)         :: kappa
 ...
 real(kind=KR2),  dimension(6,ntotal,4,2,8)   :: z2e,z2o,gz2e,gz2o,be,bo,g5xe,g5xo
 real(kind=KR2),  dimension(6,ntotal,4,2,8,1) :: xeTMP,xoTMP

 ! GMRES Variables
 integer(kind=KI),  dimension(2)               :: nGMRES
 integer(kind=KI),                             :: inverter
 integer(kind=KI), dimension(2)                :: nGMRESeig
 integer(kind=KI)                              :: itermin, iflag, isignal, mvp !moved inverter to above TW 11/17/15                 
...
! integer(kind=KI), parameter :: nop=9, nsty=10, eigstart=1, eigstep=5, ksub=6
 ...
 integer(kind=KI)                                     :: numEVPRIME,numEV,sty,iop
 integer(kind=KI)                                     :: it
...
if (myid==0) then
 print *, 'The value of kappa in testFUNC is ', kappa(1)
endif
...
!moved eigmodesofM here  by TW 11/11/17
 call eigmodesofM(gxe,gxo,be,bo,evectorRevn,evectorRodd,evectorLevn,evectorLodd,eigValExpand,numEV, &
                  u,kappa,coact,bc,vecbl,vecblinv,myid,nn,ldiv,nms,lvbc,&
                  lvcl,ib,lbd,iblv,rwdir,MRT,MRT2,nGMRESeig)
 if (myid==0) then
 print *,'calling eigmodesofMprime'
 endif                                                   
 call eigmodesofMprime(gxe,gxo,be,bo,evectorRevnPRIME,evectorRoddPRIME,eigValExpandPRIME,numEVPRIME, &
                       u,kappa,coact,bc,vecbl,vecblinv,myid,nn,ldiv,nms,lvbc,          &
                       lvcl,ib,lbd,iblv,rwdir,MRT,MRT2,nGMRESeig)
...
!commented out below call for eigmodesofM to place it before eigmodesofMprime to
!use vector from eigmodesofM in eigmodesofMprime TW 11/11/17
...
! nGMRES(2) = 40 ! must be smaller than nGMRESeig(2)
 ...
 GeeGooinv = 0.0_KR2
 resmax = 1e-7_KR2
 inverter = 6 ! use gmresdr/gmresproj, set inverters in cfgspropsmain.f90 TW
...
!if (myid==0 .and. sty==3 .and. iop==2 .and. isub==1 .and. eig==17) then
 !  print *,'HERE IS THE WEIRD DATA',sty,iop,isub,eig,it,Jtotal(1,it,isub,eig,sty,iop), &
  ! '  ',Jtotal(2,it,isub,eig,sty,iop)
!endif
 ...
 subroutine eigmodesofMprime(gxe,gxo,be,bo,evectorRevnPRIME,evectorRoddPRIME,eigValExpandPRIME,numEVPRIME, & !added g5xe/xo,be/bo TW 11/15/17
                             u,kappa,coact,bc,vecbl,vecblinv,myid,nn,ldiv,nms,lvbc,          &
                             lvcl,ib,lbd,iblv,rwdir,MRT,MRT2,nGMRES)
   use shift
   ...
   integer(kind=KI), intent(in),    dimension(:,:,:,:)   :: ib
   logical,          intent(in),    dimension(:,:)       :: lbd
   real(kind=KR2),   intent(in),    dimension(:,:,:,:,:) :: gxe,gxo,be,bo


   real(kind=KR), dimension(kmaxGMRES)          :: evalueNORM
   integer(kind=KI)                             :: irhs,j,ii,nv,gblclr,exists
   integer(kind=KI)                             :: idag, ierr
   real(kind=KR2),  dimension(2)                :: dottmp1,dottmp2
   real(kind=KR2),  dimension(6,ntotal,4,2,8)   :: oe, oo, xe, xo !placed xe,xo here TW 1/15/18                               
   real(kind=KR2),  dimension(6,ntotal,4,2,8)   :: b,tmpE,bepart,tmpEE !moved be/bo to above TW 12/8/17                              
   real(kind=KR2),  dimension(6,ntotal,4,2,8)   :: z2e, z2o, tmpO,tmpOO !BS 
   real(kind=KR2),  dimension(6,ntotal,4,2,8) :: xeTMP,xoTMP, rqeTMP,rqoTMP
   ...
   real(kind=KR2),  dimension(2)                :: tmp1, tmp2, tmp3, tmp4, & !TW
                                                   tmp5, tmp6, tmpr, rqTMPE, &
                                                   rqTMPO, rqe, rqo
...
if (myid==0) then
 print *, 'The value of kappa in eigmodesofMprime is ',kappa(1)
endif
   ! initialize/declare vairable parameters
   idag = 0; ! guarentees Mx not Mdagx
   itermin = 10
 ...
 print *,'just entered  eigmodesofMprime'
   ...
   !call z4source(z2e,z2o,nvhalf,myid) !commented out TW 11/15/17

   ! Solve Mx=b

   xe(:6,:ntotal,:4,:2,:8) = gxe(:,:,:,:,:)  ! Initial Guess changed from 0.0_KR2 TW 11/15/17                                                       
   xo(:6,:ntotal,:4,:2,:8) = gxo(:,:,:,:,:)  ! Initial Guess changed from 0.0_KR2 TW 11/15/17                                                      

   !be(:6,:ntotal,:4,:2,:8) = 0.0_KR2 !added TW 11/15/17
   !be(1,1,1,1,1) = 1.0_KR2
   !bo(:6,:ntotal,:4,:2,:8) = 0.0_KR2 !added TW 11/15/17
   !bo(1,1,1,1,1) = 1.0_KR2
   ...
   if (myid==0) then
      call cpu_time(startrhs) !TW
   endif
      call gmresdr5EIG(rwdir,be,bo,xe,xo,nGMRES,resmax,itermin,itercount,u,GeeGooinv, & !changed z2e and z2o to be and bo to match rhs of gmresdrEIG TW 11/15/17
                 iflag,idag,kappa,coact,bc,vecbl,vecblinv,myid,nn,ldiv,nms, &
                 lvbc,ib,lbd,iblv,MRT,MRT2)
   if (myid==0) then
      call cpu_time(endrhs) !TW
   endif
   if (myid==0) then
      print *, 'Time for first right hand side is', endrhs-startrhs, 'seconds' !TW
   endif
   if (myid ==0) then
...
!********Obtaining residual from evec of 1st itn of gmres-dr*********!
  !   call vecdot(ove(:,:,:,:,:,ii),ove(:,:,:,:,:,ii),tmp1,MRT2)
  !   call vecdot(ovo(:,:,:,:,:,ii),ovo(:,:,:,:,:,ii),tmp2,MRT2)
  !   tmp1 = tmp1 + tmp2
  !   tmp1(1) = sqrt(tmp1(1))
  !   ove(:,:,:,:,:,ii) = (1.0_KR2/tmp1(1)) * ove(:,:,:,:,:,ii)
  !   ovo(:,:,:,:,:,ii) = (1.0_KR2/tmp1(1)) * ovo(:,:,:,:,:,ii)


  !  oe = ove(:,:,:,:,:,ii)
  !   oo = ovo(:,:,:,:,:,ii)

     !call vecdot(oe,oe,tmp1,MRT2)
     !call vecdot(oo,oo,tmp2,MRT2)
     !tmp1 = tmp1 + tmp2
     !tmp1(1) = sqrt(tmp1(1))
     !oe = (1.0_KR2/tmp1(1)) * oe
     !oo = (1.0_KR2/tmp1(1)) * oo

     !Gamma 5 Multiplication
  !   do ieo=1,2
  !     do ibleo=1,8
  !       call gammamult(oe,oo,tmpE,tmpO,5,ieo,ibleo)
  !     enddo
  !   enddo

     !**Create the M prime matrix**!
     !Build H_oe*ove
  !   gblclr = 2
  !   call Hsingle(tmpOO,u,tmpE,idag,coact,bc,gblclr,vecbl,vecblinv,myid,nn,ldiv, &
  !                nms,lvbc,ib,lbd,iblv,MRT)
  ...
  !   tmpE = xeTMP-oneval(1,ii)*oe
  !   tmpO = xoTMP-oneval(1,ii)*oo

     !Calculate the residual, ||r|| = |M'*x-lambda*x|
  !   call vecdot(tmpE,tmpE,tmp1,MRT2)
  !   call vecdot(tmpO,tmpO,tmp2,MRT2)
  !   tmp1 = tmp1 + tmp2
  !   call cmpxsqrt(tmp1, tmp2)
    ...
    ! if (myid==0) then
     ! inquire(file=trim(rwdir(myid+1))//"eigresidual.dat", exist=exists)
      !  if (.not. exists) then
       !   open(unit=55,file=trim(rwdir(myid+1))//"eigresidual.dat",status="new",&
        !  action="write",form="formatted")
    ...
    ! if (myid==0) then
     !  open(unit=55,file=trim(rwdir(myid+1))//"eigresidual.dat",status="old",action="write",&
     !  form="formatted",position="append")
     !  write(unit=55,fmt="(i7,a6,es19.12,a6,es19.12)") ii," ",oneval(1,ii)," ",tmp2(1)
     !  close(unit=55,status="keep")
    ! endif

!************End residual calc from 1st itn of gmres-dr***************!
...
!**************TW Comment out eigresiduals.dat**********************!
     if (myid==0) then
      inquire(file=trim(rwdir(myid+1))//"eigresidual.dat", exist=exists)
        if (.not. exists) then
          open(unit=55,file=trim(rwdir(myid+1))//"eigresidual.dat",status="new",&
     ...
     if (myid==0) then
       open(unit=55,file=trim(rwdir(myid+1))//"eigresidual.dat",status="old",action="write",&
       form="formatted",position="append")
       write(unit=55,fmt="(i7,a6,es19.12,a6,es19.12)") ii," ",evalue(1,ii)," ",tmp2(1)
       close(unit=55,status="keep")
     endif

!**************End comment out eigresiduals.dat**********************!
...
! ***********************TW Comment out Norm Calc*******************!    
     !Calculate the norm of xo and xe, lines 2836-2862 added by TW 3/16/17
    ! call vecdot(xe,xe,tmp3,MRT2)
    ! call vecdot(xo,xo,tmp4,MRT2)
     
     !tmp5(1) = sqrt(tmp3(1))
     !tmp6(1) = sqrt(tmp4(1))
     !if (myid==0) then
      !  print *, "norm xe = ", " ", tmp5(1)
      !  print *, "norm xo = ", " ", tmp6(1)
     !endif
 !    tmp6(ii) = 1.0_KR2/(sqrt(tmp3(ii) +tmp4(ii)))
     !Calculate the Rayleigh Quotient of M' rho = x*M'x/x*x where x are the
     !eigenvectors of M'
!     call vecdot(xe,xeTMP,rqTMPE,MRT2)!xo*M'xo
!     call vecdot(xo,xoTMP,rqTMPO,MRT2)!xe*M'xe
 !    rqe(ii) = rqTMPE(ii)/tmp3(ii)!xe*M'xe/xe*xe
 !    rqo(ii) = rqTMPO(ii)/tmp4(ii)!xo*M'xo/xo*xo

     !Calculate the true residual norm, ||r|| = ||M'x-rho*x||
 !    rqeTMP = xeTMP-rqe(ii)*xe
 !    rqoTMP = xoTMP-rqo(ii)*xo
 !    call vecdot(rqeTMP,rqeTMP,tmp4,MRT2)
 !    call vecdot(rqoTMP,rqoTMP,tmp5,MRT2)
  !   tmp3 = tmp4 + tmp5
  !   call cmpxsqrt(tmp3,tmpr)

!*************************End Comment out ***************************!

     ! check for accuracy
     if (tmp2(1) < 1E-8) then!whytet
       numEVprime = numEVprime + 1
       ind(numEVprime) = ii  ! list of accurate modes
     endif

!****************TW Comment out writing of residuals******************!
    ! if (myid==0) then
    !  inquire(file=trim(rwdir(myid+1))//"trueresidual.dat", exist=exists)
    !    if (.not. exists) then
    !      open(unit=44,file=trim(rwdir(myid+1))//"trueresidual.dat",status="new",&
    !      action="write",form="formatted")
    ...
    ! if (myid==0) then
    !   open(unit=44,file=trim(rwdir(myid+1))//"trueresidual.dat",status="old",action="write",&
    !   form="formatted",position="append")
    !   write(unit=44,fmt="(i7,a6,es19.12,a6,es19.12)") ii," ",tmp6(ii)," ",tmpr(ii)/tmp6(ii)
    !   close(unit=44,status="keep")
    ! endif
!**************End comment out****************************************!
...
!@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
 ...
 !                  u,rwdir,coact,nms,bc,nn,iblv,ldiv,lvbc,ib,lbd,myid,MRT,MRT2,numprocs)
   ...
   resmax = 1E-7_KR2! increased to 10^-12 TW 3/14/17
