...
!BS  4/9/016 to calculate residual using directmethod
   ...
   if (.true.) then !change to .false. if you dont want this output
      if (myid==0) then
         inquire(file=trim(rwdir(myid+1))//"linresidual.dat", exist=exists)
          if (.not. exists) then
             open(unit=83,file=trim(rwdir(myid+1))//"linresidual.dat",status="new",&
      ...
      if (myid==0) then
        open(unit=83,file=trim(rwdir(myid+1))//"linresidual.dat",status="old",action="write",&
           form="formatted",position="append")
           write(unit=83,fmt="(i7,a6,es19.12)") icycle," ",xrn(1)
           close(unit=83,status="keep")
     endif
  endif ! .true.
   ...
   if (.true.) then !change to .false. if you dont want this output
      if (myid==0) then
         inquire(file=trim(rwdir(myid+1))//"rnherm.dat", exist=exists)
          if (.not. exists) then
             open(unit=93,file=trim(rwdir(myid+1))//"rnherm.dat",status="new",&
             action="write",form="formatted")
      ...
      if (myid==0) then
        open(unit=93,file=trim(rwdir(myid+1))//"rnherm.dat",status="old",action="write",&
           form="formatted",position="append")
           write(unit=93,fmt="(i7,a6,es19.12)") icycle," ",rn(1)
           close(unit=93,status="keep")
     endif
  endif ! .true.
...
!!***** added TW 1/23/18 to get residuals of lowest lying eigenpairs*******
    if (.true.) then !change to .false. if you dont need/want it
      do i = 1,10
         do ieo=1,2
           do ibleo=1,8
