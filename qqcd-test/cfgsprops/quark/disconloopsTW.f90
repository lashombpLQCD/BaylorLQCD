...
if (.false.) then
 ...
 subroutine eigmodesofM(g5xe,g5xo,evectorRevn,evectorRodd,evectorLevn,evectorLodd,eigValExpand,numEV, & !added g5xe/xo TW 11/15/17
 ...
 real(kind=KR2),  dimension(6,ntotal,4,2,8)   :: be, xe,xo,bo,x,wxo,wxe,xre,xro
 real(kind=KR2),  intent(out),  dimension(6,ntotal,4,2,8) :: g5xe,g5xo  
 ...
 be(:6,:ntotal,:4,:2,:8) = 0.0_KR2   ! Arbitrary (non-zero) rhs
 be(1,1,1,1,1) = 1.0_KR2             ! Arbitrary (non-zero) rhs
 xe(:6,:ntotal,:4,:2,:8) = 0.0_KR2  ! Initial Guess
 call gmresdrEIGBS(rwdir,be,xe,nGMRES,resmax,itermin,itercount,u,GeeGooinv, &
...
!print *,'evaleig',evalue
 ...
 bo(1,1,1,1,1) = 1.0_KR2 
  ...
  xo(:6,:ntotal,:4,:2,:8) = bo(:6,:ntotal,:4,:2,:8) + &
                            kappa(1)*tmpo(:6,:ntotal,:4,:2,:8)


 do ieo=1,2 !added TW 11/11/17, *******xo needs to be computed first!!!********
   do ibleo=1,8
      call gammamult(xe, xo ,g5xe, g5xo, 5,ieo,ibleo)
   enddo
 enddo  !end add TW 11/15/17

      gblclr = 2

      call Hsingle(wxo,u,g5xe,idag,coact,bc,gblclr,vecbl,vecblinv,myid,nn,ldiv,&
                      nms,lvbc,ib,lbd,iblv,MRT)
      ! Next build H_eo * v_o.
      gblclr = 1
      call Hsingle(wxe,u,g5xo,idag,coact,bc,gblclr,vecbl,vecblinv,myid,nn,ldiv,&
                     nms,lvbc,ib,lbd,iblv,MRT)


      wxe = g5xe - kappa(1) * wxe
      wxo = g5xo - kappa(1) * wxo
      ...
      if (myid == 0) then
         print *, 'Residual for g5xe/g5xo is ', xtmp1,' ', xtmp2,' ',  xrn(1)
      endif
...
!BS *******printing of tmp2.dat ends here
 ...
 subroutine testFUNC(u,kappa,nrhs,nGMRES,coact,bc,vecbl,vecblinv,myid,ntmqcd,nn,ldiv, &
                     nms,lvbc,lvcl,ib,lbd,iblv,rwdir,MRT,MRT2,numprocs,inverter) !added inverter TW 11/17/15                                    
 real(kind=KR),    intent(inout), dimension(:,:,:,:,:) :: u
 real(kind=KR),    intent(in),    dimension(:)         :: kappa
 ...
 ! GMRES Variables
 integer(kind=KI), intent(inout),    dimension(2) :: nGMRES
 integer(kind=KI), intent(inout)                  :: inverter
...
!moved eigmodesofM here  by TW 11/11/17
 call eigmodesofM(g5xe,g5xo,evectorRevn,evectorRodd,evectorLevn,evectorLodd,eigValExpand,numEV, &
                  u,kappa,coact,bc,vecbl,vecblinv,myid,nn,ldiv,nms,lvbc,&
                  lvcl,ib,lbd,iblv,rwdir,MRT,MRT2,nGMRESeig)
 if (myid==0) then
 print *,'calling eigmodesofMprime'
 endif                                                   
 call eigmodesofMprime(g5xe,g5xo,evectorRevnPRIME,evectorRoddPRIME,eigValExpandPRIME,numEVPRIME, &
                       u,kappa,coact,bc,vecbl,vecblinv,myid,nn,ldiv,nms,lvbc,          &
                       lvcl,ib,lbd,iblv,rwdir,MRT,MRT2,nGMRESeig)
...
!endif
 ...
 subroutine eigmodesofMprime(g5xe,g5xo,evectorRevnPRIME,evectorRoddPRIME,eigValExpandPRIME,numEVPRIME, & !added g5xe/xo TW 11/15/17
                             u,kappa,coact,bc,vecbl,vecblinv,myid,nn,ldiv,nms,lvbc,          &
                             lvcl,ib,lbd,iblv,rwdir,MRT,MRT2,nGMRES)
   use shift
   ...
   integer(kind=KI), intent(in),    dimension(:,:,:,:)   :: ib
   logical,          intent(in),    dimension(:,:)       :: lbd
   real(kind=KR2),   intent(in),    dimension(:,:,:,:,:) :: g5xe,g5xo
   ...
   real(kind=KR2),  dimension(6,ntotal,4,2,8)   :: xe, xo, oe, oo
   real(kind=KR2),  dimension(6,ntotal,4,2,8)   :: b,tmpE,bepart,tmpEE,be,bo !BS
...
if (myid==0) then
 ...
 print *,'just entered  eigmodesofMprime'
   ...
   xe(:6,:ntotal,:4,:2,:8) = g5xe  ! Initial Guess changed from 0.0_KR2 TW 11/15/17                                                       
   xo(:6,:ntotal,:4,:2,:8) = g5xo  ! Initial Guess changed from 0.0_KR2 TW 11/15/17                                                      
   ...
   if (myid==0) then
      ...
      call gmresdr5EIG(rwdir,z2e,z2o,xe,xo,nGMRES,resmax,itermin,itercount,u,GeeGooinv, & !changed z2e and z2o to be and bo to match rhs of gmresdrEIG TW 11/15/17
                 iflag,idag,kappa,coact,bc,vecbl,vecblinv,myid,nn,ldiv,nms, &
                 lvbc,ib,lbd,iblv,MRT,MRT2)
   if (myid==0) then
