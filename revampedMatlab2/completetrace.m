      		...
      		%for ksub = 1:nsub+1  - Removed for loop over ksub -PL 
				...
				for iop=1:5
          			if (iop == 5) % scalar
            			...
            			%esgps_raw(rhs,ieig,ksub,iop) = esg_raw(rhs,ieig,iop) + ps_raw(rhs,ksub,iop) - nsg_raw(rhs,iop) ...
													  %+(z(:,rhs)' * gammaX(gammaX(xp(:,ksub),5),iop)); -PL
...
%*****************************************************************
    	...
    	for ieig = 1:neigH
      		%for ksub = 1:nsub+1 - Removing for loop over ksub -PL 
				...
				for iop=1:5
          			if (iop == 5) % scalar
            			%esgpolylatest_raw(rhs,ieig,ksub,iop) = esg_raw(rhs,ieig,iop) + poly_raw(rhs,ksub,iop) ... 
																%- nsg_raw(rhs,iop) + (z(:,rhs)' * gammaX(xp(:,ksub),5)); -PL
            			esgpolylatest_raw(rhs,ieig,iop) = esg_raw(rhs,ieig,iop) + poly_raw(rhs,iop) - nsg_raw(rhs,iop) + (z(:,rhs)' * gammaX(xp(:),5));
          			else
		   				%esgpolylatest_raw(rhs,ieig,ksub,iop) = esg_raw(rhs,ieig,iop) + poly_raw(rhs,ksub,iop) ...
																%- nsg_raw(rhs,iop) + (z(:,rhs)' * gammaX(gammaX(xp(:,ksub),5),iop)); -PL
